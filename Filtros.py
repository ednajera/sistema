    ...
    if not df.empty:
        ...
        if st.button("Generar Archivos para Tutores"):
            with st.spinner('Generando archivos, por favor espera...'):
                zip_file_name = "archivos_tutores.zip"
                with zipfile.ZipFile(zip_file_name, 'w') as zip_file:
                    for index, row in df[mask].iterrows():
                        ...
                        doc_file = f"{row['Estudiante']}.docx"
                        template.save(doc_file)
                        pdf_file = f"{row['Estudiante']}.pdf"
                        convert(doc_file, pdf_file)

                        # Agregar el archivo PDF al archivo ZIP sin guardarlo localmente
                        pdf_data = open(pdf_file, "rb").read()
                        zip_file.writestr(f"{row['Estudiante']}.pdf", pdf_data)
                zip_data = open(zip_file_name, "rb").read()
                zip_download_link = f'[Descargar ZIP](data:application/zip;base64,{base64.b64encode(zip_data).decode()})'
        ...
        if st.button("Generar Archivos para Docentes"):
            with st.spinner('Generando archivos, por favor espera...'):
                zip_file_name = "archivos_docentes.zip"
                with zipfile.ZipFile(zip_file_name, 'w') as zip_file:
                    for index, row in df[mask].iterrows():
